...
-- Union the 12 month tables, create table of joined month tables.
...
--FINDINGS: California, $13,711,290
...
WITH sales_products_rows AS (
	SELECT 
		Product,
		[Quantity Ordered],
		[Price Each],
		CONVERT(FLOAT,[Quantity Ordered]) * [Price Each] AS sale_product_order
	FROM Project_Sales_Data_Three_Ways..sales_data
	)
	,
group_products AS (
	SELECT
		SUM(CASE WHEN Product LIKE '%Batteries%' THEN sale_product_order ELSE 0 END) AS batteries,
		SUM(CASE WHEN Product LIKE '%Headphones' THEN sale_product_order ELSE 0 END) AS headphones,
		SUM(CASE WHEN Product LIKE '%Cable' THEN sale_product_order ELSE 0 END) AS cables,
		SUM(CASE WHEN Product LIKE '%Monitor' THEN sale_product_order ELSE 0 END) AS monitors,
		SUM(CASE WHEN Product LIKE '% Phone' OR Product LIKE 'iPhone' THEN sale_product_order ELSE 0 END) AS phones,
		SUM(CASE WHEN Product LIKE '%Laptop%' THEN sale_product_order ELSE 0 END) AS laptops,
		SUM(CASE WHEN Product LIKE '%TV' THEN sale_product_order ELSE 0 END) AS televisions,
		SUM(CASE WHEN Product LIKE '%Machine' OR Product LIKE '%Dryer' THEN sale_product_order ELSE 0 END) AS home_appliances
	FROM sales_products_rows
	GROUP BY Product
	)

---- Sum the number of orders within each category.
SELECT
	ROUND(SUM(batteries), 0) AS batteries_sales,
	ROUND(SUM(headphones), 0) AS headphones_sales,
	ROUND(SUM(cables), 0) AS cables_sales,
	ROUND(SUM(monitors), 0) AS monitors_sales,
	ROUND(SUM(phones), 0) AS phones_sales,
	ROUND(SUM(laptops), 0) AS laptops_sales,
	ROUND(SUM(televisions), 0) AS televisions_sales,
	ROUND(SUM(home_appliances), 0) AS home_appliances_sales
FROM group_products;
